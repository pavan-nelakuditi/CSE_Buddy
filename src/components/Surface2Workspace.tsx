...
export function Surface2Workspace({ specContext, onExportSuccess }: Props): ReactElement {
  const [mode, setMode] = useState<Surface2Mode>('edit');
  ...
  function getStepOperation(step: FlowStep): SpecOperation | undefined {
    return operationsById.get(step.operationId);
  }

  function getBindingLabel(step: FlowStep, binding: FlowStep['bindings'][number]): string {
  ...
  function getReviewWarnings(): string[] {
    ...
    const exampleBindingCount = draft.flow.steps.reduce(
      (count, step) => count + step.bindings.filter((binding) => binding.source === 'example').length,
      0
    );
    const literalBindingCount = draft.flow.steps.reduce(
      (count, step) => count + step.bindings.filter((binding) => binding.source === 'literal').length,
      0
    );
    const syntheticOperationCount = draft.flow.steps.filter((step) => getStepOperation(step)?.syntheticOperationId).length;
    const stepsWithoutExtracts = draft.flow.steps.filter((step) => step.extract.length === 0).length;

    if (exampleBindingCount > 0) {
      warnings.push(`${exampleBindingCount} field(s) use examples.`);
    }
    if (literalBindingCount > 0) {
      warnings.push(`${literalBindingCount} field(s) use literal values.`);
    }
    if (syntheticOperationCount > 0) {
      warnings.push(`${syntheticOperationCount} operationId(s) were generated by Surface 1.`);
    }
    if (stepsWithoutExtracts > 0) {
      warnings.push(`${stepsWithoutExtracts} step(s) do not extract response values.`);
    }

    return warnings;
  }

  function handleReviewFlow(): void {
    if (!draft) {
      setError('Generate a smoke flow before review.');
      return;
    }
    if (draft.flow.steps.length === 0) {
      setError('Add at least one step before review.');
      return;
    }
    setMode('review');
    setError('');
    setStatus('Review the smoke flow before export.');
  }
    ...
    <section className="panel surface2-shell">
      <div className="panel-header">
        ...
        <div className="inline-actions">
          ...
          {mode === 'edit' ? (
            <button type="button" className="primary" onClick={handleReviewFlow} disabled={!draft}>
              Review flow
            </button>
          ) : (
            <>
      ...
      <div className="surface-guide-grid">
        ...
        <div className="sequence-card surface-guide-card">
          <div className="sequence-card-top">
            ...
            <div>
              <strong>Confirm flow</strong>
              <p>Preview the journey before export.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {draft && mode === 'review' ? (
        <div className="surface2-review">
          <section className="surface2-review-main">
            <div className="panel-header">
              <div>
                <h3>Review smoke flow</h3>
                <p>{draft.flow.name}</p>
              </div>
              <span className="surface2-mini-pill">{draft.flow.steps.length} steps</span>
            </div>

            <div className="surface2-review-steps">
              {draft.flow.steps.map((step, index) => {
                const operation = getStepOperation(step);
                return (
                  <div key={step.id} className="surface2-review-step">
                    <span className="surface2-step-number static">{index + 1}</span>
                    <div>
                      <strong>{step.name?.trim() || operation?.summary || step.operationId}</strong>
                      <p>
                        <span className={`method-chip method-${operation?.method.toLowerCase() ?? 'get'}`}>{operation?.method ?? 'OP'}</span>
          ...
          <aside className="surface2-review-side">
            ...
            <section className="surface2-review-card">
              ...
              {draft.flow.steps.some((step) => step.bindings.some((binding) => binding.source === 'prior_output')) ? (
                <ul className="surface2-review-list">
                  {draft.flow.steps.flatMap((step) =>
                    step.bindings
                      .filter((binding) => binding.source === 'prior_output')
                      .map((binding) => <li key={`${step.id}-${binding.fieldKey}`}>{getBindingLabel(step, binding)}</li>)
                  )}
                </ul>
              ) : (
                <p>No prior-output bindings.</p>
              )}
            </section>

            <section className="surface2-review-card">
              <h3>Extracts</h3>
              {draft.flow.steps.some((step) => step.extract.length > 0) ? (
                <ul className="surface2-review-list">
                  {draft.flow.steps.flatMap((step) =>
                    step.extract.map((extract) => (
                      <li key={`${step.id}-${extract.id}`}>
                        {(step.name?.trim() || step.operationId)} extracts <code>{extract.variable}</code> from <code>{extract.jsonPath}</code>
                      </li>
                    ))
                  )}
                </ul>
              ) : (
                <p>No extracts selected.</p>
              )}
            </section>

            <section className="surface2-review-card">
              <h3>Review notes</h3>
              {getReviewWarnings().length > 0 ? (
                <ul className="surface2-review-list">
                  {getReviewWarnings().map((warning) => (
